...
- Fix where :meth:`DataFrame.loc` and :meth:`Series.loc` setitem with an integer row key on a :class:`DatetimeIndex` (or :class:`TimedeltaIndex` / :class:`PeriodIndex`) could silently add a row and cast the index to object dtype; these now raise instead. Use :meth:`DataFrame.iloc` or :meth:`Series.iloc` for positional indexing, or use a label with :meth:`DataFrame.loc` (:issue:`22040`)
-
